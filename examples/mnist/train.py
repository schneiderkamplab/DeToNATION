# A simple example of PyTorch distributed training across multiple nodes using FlexDeMo
# Adapted from: https://leimao.github.io/blog/PyTorch-Distributed-Training/
...
def main():
    ...
    if resume == True:
        ...
        fsdp_model.load_state_dict(torch.load(model_filepath, map_location=map_location))
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % 10 == 0:
            if local_rank == 0:
                accuracy = evaluate(model=fsdp_model, device=device, test_loader=test_loader)
                torch.save(fsdp_model.state_dict(), model_filepath)
                print("-" * 75)
